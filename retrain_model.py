"""
===============================================================================
RETRAIN MODEL WITH FIXED FEATURE DIMENSIONS
Fixes the dimension mismatch: 845 (expected) vs 771 (actual)
===============================================================================

ROOT CAUSE:
-----------
The model was trained with 845 input features:
  - 768 from BERT embeddings
  - 74 from skill multi-hot encoding
  - 3 from structured features (experience, skill_count, text_length)
  
But during inference, transform_single_resume was only creating:
  - 768 from BERT embeddings
  - 0 from skills (when no skills matched frequent_skills)
  - 3 from structured features
  = 771 features ❌

FIX:
----
The transform_single_resume method must ALWAYS create a 74-dimensional skill
vector (matching the trained skill_binarizer), even when no skills are found.
"""
...
CONFIG = {
    ...
    'val_size': 0.1,
    'random_seed': 42,
    'min_skill_frequency': 3,  # Lowered from 5 to capture more skills
...
def train_hybrid_ann(X_train, y_train, X_val, y_val, num_classes, input_dim):
    """Train Hybrid ANN model with correct input dimension"""
    print("\n" + "="*80)
...
def verify_feature_dimensions(feature_engineer, bert_extractor):
    """
    CRITICAL: Verify that transform_single_resume produces the same
    number of features as create_hybrid_features used during training.
    """
    print("\n" + "="*80)
    print("VERIFYING FEATURE DIMENSIONS")
    print("="*80)
    ...
    print(f"\n✓ BERT embedding shape: {bert_embedding.shape}")
    
    # Transform using single resume method
    ...
    if actual_dim == expected_dim:
        print(f"\n✅ SUCCESS: Feature dimensions match!")
        ...
        print(f"   Expected: {expected_dim}")
        print(f"   Actual:   {actual_dim}")
        print(f"   Difference: {expected_dim - actual_dim}")
        return False
...
def main():
    ...
    print("\n" + "="*80)
    print("🔄 RETRAINING MODEL WITH FIXED FEATURE DIMENSIONS")
    print("="*80)
    ...
    print(f"\n📐 Training feature dimensions: {hybrid_features.shape[1]}")
    print(f"   - BERT: 768")
    print(f"   - Skills: {feature_engineer.skill_binarizer.classes_.shape[0]}")
    print(f"   - Structured: 3")
    ...
    # Phase 8: CRITICAL - Verify feature dimensions match
    dimensions_ok = verify_feature_dimensions(feature_engineer, bert_extractor)
    
    if not dimensions_ok:
        print("\n❌ ABORTING: Feature dimensions don't match!")
        print("   The model will fail during inference.")
        return
    ...
    # Backup old model
    if os.path.exists('models/bert_ann_model.h5'):
        print("\n📦 Backing up old model...")
        import shutil
        shutil.copy('models/bert_ann_model.h5', 'models/bert_ann_model_backup.h5')
        shutil.copy('models/feature_engineer.pkl', 'models/feature_engineer_backup.pkl')
    ...
    print(f"\nKey Fixes:")
    print(f"  ✓ Feature dimensions verified: {hybrid_features.shape[1]}")
    print(f"  ✓ Skill binarizer: {feature_engineer.skill_binarizer.classes_.shape[0]} skills")
    print(f"  ✓ transform_single_resume will produce matching dimensions")
    print("="*80)
